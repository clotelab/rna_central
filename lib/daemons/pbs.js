...
module.exports = {
  ...
  submit_job: function(webserver, job, callback) {
    ...
    // return this.build_workspace(webserver, job).then(function(directory) {
    //
    // });
  ...
  build_workspace: function(webserver, job) {
    var path = path.join("../../workspace", job._id);
    
    return fs.mkdirAsync(path).return(path);
  },
  
  render_job_file: function(folder, webserver, job) {
    return fs.readFileAsync("../templates/pbs_header.mustache").then(function(data) {
      return mustache.render(data, {
        nodes: "1:clotelab",
        walltime: "24:00:00",
        output_name: path.join(folder, job._id + ".log"),
        error_name: path.join(folder, job._id + ".err"),
        queue: "webserver",
        commands: "MEOW"
      });
    });

...
var pbs = module.exports = {
  ...
  on_tick: function() {
    if (process.env._system_type === "Linux" && shell.which("qstat")) {
      child_process.execAsync("qstat").bind(this).spread(function(stdout) {
        var qstat_output = this.parse_qstat(stdout);

        this.sync_db(qstat_output);
      });
    } else {
  ...
  sync_db: function(qstat_output) {
    bluebird.all(_.map(qstat_output, function(qstat_entry) {
      return daemon().warehouse.Job.update_by_queue_id(qstat_entry.queue_id, qstat_entry.state);
    })).then(function(searched_jobs) {
      var updated_jobs = _.compact(searched_jobs);

      if (updated_jobs.length > 0) {
        debug("A total of " + updated_jobs.length + " jobs were updated");
      }
    });
  },
  ...
  render_job_file: function(workspace, webserver, job) {
    ...
    var commands      = webserver.generate_command(job.data, job);

    debug(job.nickname + " generated the following command:");
    debug(commands);
  ...
  qsub_script_file: function(script_file) {
    ...
    return child_process.execAsync(shell_command).spread(function(stdout) {
      return stdout.trim();
    });
  },

...
var proto = module.exports = function(options) {
  ...
  router.get("/jobs/:nickname", function(req, res, next) {
    ...
    // Maybe put the handle_complete_job in here instead...
    if (job) {
      ...
      if (job.state === "complete") {
        router.finished_job_route.call(job, req, res, next);
      } else {
...
_.extend(proto, {
  ...
  display_results: function(finished_job_route) {
    this.finished_job_route = finished_job_route;
  },

  finished_job_route: function(job, req, res, next) {
    res.sendFile(job.workspace_file(".log"));
  },

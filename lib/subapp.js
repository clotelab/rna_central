...
var proto = module.exports = function(options) {
  ...
  router.param("nickname", function(req, res, next, nickname) {
    req.state.db.Job.findOneAsync({
      ...
      req.state.job = job;
    }).catch(console.log.bind(console)).finally(next);
  });
...
_.extend(proto, {
  ...
  organize_stack: function() {
    this.__proto__.stack = _.chain(this.stack.length).
                             range().
                             zip(this.stack).
                             map(function(zipper) {
  ...
  add_tab: function(page_data) {
    ...
    if (_.include(page_data.path, "/submit_job")) {
      this.add_form_post_route(page_data);
    }
  },

  add_form_post_route: function(page_data) {
    var router = this;

    router.post(page_data.path, function(req, res, next) {
      router.form.handle(req, {
        success: function(form) {
          if (router.form_is_valid(form)) {
            router.handle_valid_form(form, req, res, next);
          } else {
            router.handle_invalid_form(page_data, form, req, res);
          }
        },
        other: function(form) {
          router.render_invalid_form(page_data, form, res);
        }
      });
    });
  },

  handle_valid_form: function(form, req, res, next) {
    req.state.db.Job.create_job(req.state.subapp.id, form.data).then(function(job) {
  ...
  handle_invalid_form: function(page_data, form, req, res) {
    ...
    this.render_invalid_form(page_data, form, res);
  },

  render_invalid_form: function(page_data, form, res) {
    res.render(page_data.template, { page_data: page_data, previous_form: form });
  }
});

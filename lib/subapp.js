...
var proto = module.exports = function(options) {
  ...
  router.use(function(req, res, next) {
    _.extend(res.locals, {
      ...
      scoped_url:   function(string) {
        return path.join(req.rna_central.base_prefix || "", req.baseUrl, string || "");
      },
      public_url:   function(string) {
        return path.join(req.rna_central.base_prefix || "", string || "");
      }
    });
  ...
  router.param("nickname", function(req, res, next, nickname) {
    req.rna_central.warehouse.Job.findOneAsync({
      webserver_name: req.rna_central.subapp.id,
      nickname: nickname
    }).then(function(job) {
      router.debug("Pulling info for job " + nickname);

      req.rna_central.job = job;
    }).finally(next);
  });

  router.get("/jobs/:nickname", function(req, res, next) {
    if (req.rna_central.job) {
      if (req.rna_central.job.state === "complete") {
        res.send(path.join(req.rna_central.job.workspace_path, req.rna_central.job.nickname + ".log"));
      } else {
        res.json(req.rna_central.job);
      }
    } else {
      next();
    }
...
_.extend(proto, {
  ...
  generate_command: function(data) {
    throw new Error("Webserver instances must provide a function to pbs_command");
  },

  finish_job: function(complete_job_handler) {
    this.handle_complete_job = complete_job_handler;
  },

  handle_complete_job: function() {
    throw new Error("Webserver instances must provide a function to finish_job");
  },

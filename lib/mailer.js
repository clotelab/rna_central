...
var proto = module.exports = function mailer(transport) {
	if (!loaded && !transport) {
	  throw new Error("Trying to load the cached mailer before is was ever initialized");
	}

	if (!loaded) {
		loaded                = true;
    transporter           = nodemailer.createTransport(transport);
    proto.__proto__       = transporter.__proto__;
    transporter.__proto__ = proto;
...
_.extend(proto, {
	send_mail: function(options) {
	  if (!_.includes(["development"], process.env.NODE_ENV)) {
	    return this.sendMailAsync(_.extend({
	...
	send_mail_about_job: function(job) {
		fs.readFileAsync(

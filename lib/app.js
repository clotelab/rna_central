...
var daemon        = require("./daemon")(pbs, "*/30 * * * * *");
...
app.use(function(req, res, next) {
  req.base_prefix = process.env.BASE_PREFIX || "";
...
app.param("webserver", function(req, res, next, webserver) {
  try {
    ...
    req.subapp      = require(server_path);
...
app.use("/:webserver", function(req, res, next) {
  if (_.isUndefined(req.subapp) || req.subapp.active !== true) {
    if (req.subapp.active !== true) {
      debug(req.subapp.id + " is not flagged as an active webserver, ignoring all requests");
    }

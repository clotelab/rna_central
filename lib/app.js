...
debug(colors.inverse.underline("Using configuration settings in %s.json"), process.env.NODE_ENV || "default");
...
app.use(logger("dev"));
app.use(cookie_parser(config.get("session_secret")));
app.use(session({
...
app.use("/:webserver", function(req, res, next) {
  if (_.isUndefined(req.rna_central.subapp) || req.rna_central.subapp.active !== true) {
    if (req.rna_central.subapp.active !== true) {
      debug("%s is not flagged as an active webserver, ignoring all requests", req.rna_central.subapp.id);
    }

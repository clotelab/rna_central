...
job_schema.pre("save", function(next) {
  if (_.includes(this.modifiedPaths(), "state")) {
    debug(util.format("Job %s (%s) state updating to %s", this.nickname, this.queue_id, this.state));
  }
...
job_schema.post("save", function(job) {
  ...
  if (job.state === "complete") {
    debug(util.format("Job %s (%s) is now complete!", job.nickname, job.queue_id));
...
_.extend(job_schema.methods, {
  ...
  submit: function() {
    ...
    return daemon().queue.submit_job(job.webserver, job).then(function(queue_id) {
      debug(util.format("Job %s (%s) submitted successfully, flagging as queued", job.nickname, queue_id));

      return job.updateAsync({ queue_id: queue_id, state: "queued" });
    }).catch(function(err) {
      debug(util.format("Job %s (%s) had an issue, marking as error", job.nickname));

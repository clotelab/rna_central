...
var session_socket = require("../sockets");
...
job_schema.post("save", function(job) {
  ...
  if (job.state === "complete" && process.env.NODE_ENV !== "test") {
    ...
    session_socket().io.to(job.nickname).emit("done", job.nickname);
    ...
    fs.readdirAsync(job.workspace_path).then(function(files) {
      ...
      job.webserver.handle_complete_job.call(job, full_file_paths);
    });
  }
});
...
_.extend(job_schema.methods, {
  ...
  submit: function() {
    ...
    return daemon().queue.submit_job(job.webserver, job).then(function(queue_id) {
      ...
      debug(colors.underline("Job %s (%s) had an issue, marking as error"), job.nickname);

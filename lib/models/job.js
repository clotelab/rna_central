...
job_schema.pre("update", function(next) {
  next();
});
...
_.extend(job_schema.methods, {
  ...
  submit: function() {
    ...
    return daemon().queue.submit_job(job.webserver, job).then(function(queue_id) {
      debug("Job " + job.nickname + " submitted successfully, flagging as queued");

      return job.updateAsync({ queue_id: queue_id, state: "queued" });
    }).catch(function(err) {
      debug("Job " + job.nickname + " had an error, marking as such");

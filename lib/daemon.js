"use strict";

var bluebird = require("bluebird");
var debug    = require("debug")("rna_central:daemon");
var config   = require("config");
var CronJob  = require("cron").CronJob;

function Daemon() {
  this.queue     = {};
  this.warehouse = {};
}

Daemon.prototype.config = function(warehouse, daemon_functions) {
  if (process.env.NODE_ENV !== "test") {
    var on_tick_freq = config.get("on_tick_freq");
    this.queue       = new CronJob(on_tick_freq, daemon_functions.on_tick, _.identity, true);

    debug(on_tick_freq);
  }

  this.warehouse                   = warehouse;
  this.queue.__proto__             = _.extend({}, daemon_functions);
  this.queue.__proto__.__proto__   = CronJob.prototype;
  this.queue.__proto__.constructor = CronJob;

  bluebird.promisifyAll(this.queue);

  return this;
};

var daemon = module.exports = new Daemon();

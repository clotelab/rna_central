...
var proto = module.exports = function(options) {
  ...
  if (_.isArray(options.tabs)) {
    ...
    router.add_tab({ title: "Server",  path: "/submit_job",       template: "server", meta: "server" });
...
_.extend(proto, {
  ...
  add_tab: function(tab_settings) {
    ...
    if (tab_settings.meta === "server") {
      debug("this.add_form_post_route(tab_settings)");
      
      this.add_job_submission_route(tab_settings);
    }
  },
  
  add_job_submission_route: function(server_tab) {
    var router = this;
